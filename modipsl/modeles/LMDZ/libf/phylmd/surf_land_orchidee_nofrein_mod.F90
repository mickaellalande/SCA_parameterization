!
! $Id: surf_land_orchidee_nofrein_mod.F90 3102 2017-12-03 20:27:42Z oboucher $
!
!
MODULE surf_land_orchidee_nofrein_mod
#ifdef ORCHIDEE_NOFREIN
!
! This module controles the interface towards the model ORCHIDEE.
!
! Compatibility with ORCHIDIEE :
! This module is compiled only if cpp key ORCHIDEE_NOFREIN is defined. 
! The current version can be used with ORCHIDEE/trunk from revision 3525-4465 
    ...
    IF (debut) THEN
       ...
       IF (knon > 0) THEN
         ...
         CALL intersurf_initialize_gathered (itime+itau_phy-1, nbp_lon, nbp_lat, knon, ktindex, dtime, &
               ...
               lon_scat, lat_scat, q2m, t2m, z0h_new)
#endif         
       ENDIF
...
!  swdown_vrai(1:knon) = swnet(1:knon)/(1. - albedo_keep(1:knon))
    ...
    IF (knon > 0) THEN
       ...
       CALL intersurf_main_gathered (itime+itau_phy, nbp_lon, nbp_lat, knon, ktindex, dtime,  &
            ...
            lon_scat, lat_scat, q2m, t2m, z0h_new(1:knon), coszang=yrmu0(1:knon))
#endif       
    ENDIF
...
!
#endif
END MODULE surf_land_orchidee_nofrein_mod
