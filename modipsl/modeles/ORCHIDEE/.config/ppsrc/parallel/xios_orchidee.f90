...
!
!>\BRIEF   This module contains the initialization and interface to the 1 code.
!!
!!\n DESCRIPTION: This module contains the interface for the use of the 1 code. All call to 1 are done in this module.
!!                Revision 965 of 1/trunk or later is needed. This version is also called XIOS2. 
!!                Older revisions and XIOS1 can not be used.
!!                
!!                Summury of subroutines
!!                      xios_orchidee_comm_init       : First call to 1 to get the MPI communicator 
!!                      xios_orchidee_init            : Initialize variables needed for use of 1 
!!                                                      Deactivation of fields not calculated due specific run options
!!                      xios_orchidee_update_calendar : Update the calandar in 1
!!                      xios_orchidee_finalize        : Last call to 1 for finalization
!!                      xios_orchidee_send_field      : Interface to send fields with 1, 2 or 3 dimensions to 1
...
!!
!!                It is only possible to use XIOS2. Note that compilation must be done with the preprocessing key 1 
!!                and 1. Compiling without these keys makes it impossible to activate 1. 
!!                To activate running using 1, the flag XIOS_ORCHIDEE_OK=y must be set in run.def and the file iodef.xml must exist.  
!!
...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/tags/ORCHIDEE_2_0/ORCHIDEE/src_parallel/xios_orchidee.f90 $
!! $Date: 2018-09-27 18:02:14 +0200 (Thu, 27 Sep 2018) $
!! $Revision: 5451 $
!! \n
!_ ================================================================================================================================
...
MODULE xios_orchidee
  ...
  !
  LOGICAL, PUBLIC, SAVE           :: xios_orchidee_ok=.TRUE.     !! Use 1 for diagnostic files
  !$OMP THREADPRIVATE(xios_orchidee_ok)
  REAL(r_std), PUBLIC, SAVE       :: xios_default_val=0          !! Default value (missing value) used in 1. The value 0 will be overwritten with the value taken from 1.
  ...
  CHARACTER(len=*),PARAMETER      :: id="client"           !! Id for initialization of ORCHIDEE in 1
  ...
  !!
  !>\BRIEF         Send a field to 1.
  !!
  !! DESCRIPTION  :\n Send a field to 1. The field can have 1, 2 or 3 dimensions.
...
CONTAINS
  ...
  !!
  !! DESCRIPTION  :\n First call to 1 to get the MPI communicator. 
  !!                  Note that it is 1 that initialize the MPI communicator.
  !!                  This subroutine is only called in ORCHIDEE offline mode. When running in coupled mode, the 
  !!                  atmospheric model must initlialize 1 at the same time as initializing MPI. 
  !! \n
  !_ ================================================================================================================================
  ...
  !!
  !>\BRIEF         Initialize variables needed for use of 1.
  !!
  !! DESCRIPTION  :\n Initialization of specific varaiables needed to use 1 such as model domain and time step. 
  !!
  ...
  SUBROUTINE xios_orchidee_init(MPI_COMM_ORCH,                   &
    ...
    !Config Desc  = Use 1 for writing diagnostics file
    !Config If    = 
    !Config Def   = y 
    !Config Help  = Compiling and linking with 1 library is necessary. 
    !Config Units = [FLAG]
    ...
    !
    !! 1. Set date and calendar information on the format needed by 1
    !

    ! Get the calendar from IOIPSL and modify the string to correspond to what 1 expects
    ...
    IF (xios_orchidee_ok .AND. is_omp_root) THEN
       ...
       !
       !! 5. Get the default value (missing value) used by 1. This value is set in field_def_orchidee.xml
       !
  ...
  !!
  !>\BRIEF          Update the calandar in 1.
  !!
  !! DESCRIPTION  :\n Update the calendar in 1 : let 1 know that ORCHIDEE avanced one time-step.
  ...
  SUBROUTINE xios_orchidee_update_calendar(itau_sechiba)
    ...
    INTEGER(i_std)        :: itau_xios            !! Current time step for 1
  ...
  !!
  !! DESCRIPTION  :\n This subroutine finalizes the orchidee context without finalizing 1. In coupled mode, the atmospheric
  !!                  modele must finalize 1. This subroutine is called in the end of the execution of ORCHIDEE only in 
  !!                  coupeld mode.
  !!                  
  !! \n
  !_ ================================================================================================================================
  ...
  !!
  !>\BRIEF         Last call to 1 for finalization.
  !!
  !! DESCRIPTION  :\n Last call to 1 for finalization of the orchidee context and 1.
  !!                  This subroutine is called only when ORCHIDEE is run in offline mode. In coupled mode it is the atmospheric
  !!                  model that finalizes 1. In that case, the context orchidee must be finalized using the 
  !!                  subroutine xios_orchidee_context_finalize
  !!                  
  !! \n
  !_ ================================================================================================================================
  ...
  !!
  !>\BRIEF          Subroutine for sending 1D (array) fields to 1.
  !!
  !! DESCRIPTION  :\n Send one field to 1. This is the interface for 1D fields (array).
  ...
  SUBROUTINE xios_orchidee_send_field_r1d(field_id,field)
    ...
    IF (xios_orchidee_ok) THEN
       ...
       ! All master threads send the field to 1
       IF (is_omp_root) THEN
  ...
  !!
  !>\BRIEF          Subroutine for sending 2D fields to 1.
  !!
  !! DESCRIPTION  :\n Send one field to 1. This is the interface for 2D fields.
  !!                  NB! This subroutine should not be called directly. Use interface xios_orchidee_send_field.
  !!
  !! \n
  !_ ================================================================================================================================
  ...
  !!
  !>\BRIEF          Subroutine for sending 3D fields to 1. 
  !!
  !! DESCRIPTION  :\n Send one field to 1. This is the interface for 3D fields.
  !!                  NB! This subroutine should not be called directly. Use interface xios_orchidee_send_field.
  !!
  !! \n
  !_ ================================================================================================================================
  ...
  !!
  !>\BRIEF          Subroutine for sending 4D fields to 1. 
  !!
  !! DESCRIPTION  :\n Send one field to 1. This is the interface for 4D fields.
  !!                  NB! This subroutine should not be called directly. Use interface xios_orchidee_send_field.
  !!
  !! \n
  !_ ================================================================================================================================
  ...
  !!
  !>\BRIEF          Subroutine for sending 5D fields to 1. 
  !!
  !! DESCRIPTION  :\n Send one field to 1. This is the interface for 5D fields.
  !!                  NB! This subroutine should not be called directly. Use interface xios_orchidee_send_field.
  !!
  !! \n
  !_ ================================================================================================================================
  ...
  !!
  !>\BRIEF          Subroutine for receiving 1D (kjpindex) fields to 1. 
  !!
  !! DESCRIPTION  :\n 
  !!
  !! \n
  ...
  SUBROUTINE xios_orchidee_recv_field_r1d(field_id,field)
    ...
    IF (xios_orchidee_ok) THEN
       ...
       ! All master threads receive the field from 1
       IF (is_omp_root) THEN
  ...
  !! SUBROUTINE   : xios_orchidee_recv_field_r2d
  !!
  !>\BRIEF          Subroutine for receiving 2D(kjpindex and 1 vertical axe) fields to 1. 
  !!
  !! DESCRIPTION  :\n 
  !!
  !! \n
  ...
  SUBROUTINE xios_orchidee_recv_field_r2d(field_id,field)
    ...
    IF (xios_orchidee_ok) THEN
       ...
       ! All master threads recieve the field from 1
       IF (is_omp_root) THEN
          CALL xios_recv_field(field_id,field_mpi)
  ...
  !!
  !>\BRIEF          Subroutine for receiving 3D(kjpindex and 2 vertical axes) fields to 1. 
  !!
  !! DESCRIPTION  :\n 
  !!
  !! \n
