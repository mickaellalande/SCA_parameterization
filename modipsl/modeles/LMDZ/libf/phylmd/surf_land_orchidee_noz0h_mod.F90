!
MODULE surf_land_orchidee_noz0h_mod
#ifdef ORCHIDEE_NOZ0H
!
! This module controles the interface towards the model ORCHIDEE.
!
! Compatibility with ORCHIDIEE :
! This module is compiled only if CPP key ORCHIDEE_NOZ0H is defined.
! This version can be used with ORCHIDEE/trunk revision 2961-3525.
! It is possible to use this interface with later versions of ORCHIDEE/trunk than 3525 but 
! in that case the option ROUGH_DYN=y in ORCHIDEE should not be set. 
! This interface can also be used with ORCHIDEE/trunk revision 1078-2960 if changing 
! coszang=yrmu0 into sinang=yrmu0 at 2 places later below in this module. 
!
...
!   z0h_new      surface roughness, it is a copy from z0_new
...
!****************************************************************************************
    ...
    IF (ifl_pbltree == 1) THEN
       abort_message='Pb de coherence: cette interface vers ORCHIDEE ne peut pas etre utilise avec ifl_pbltree'
    ...
    IF (debut) THEN
       ...
       IF (knon > 0) THEN
          ...
          CALL intersurf_main (itime+itau_phy-1, nbp_lon, nbp_lat, knon, ktindex, dtime, &
               ...
               lon_scat, lat_scat, q2m, t2m, coszang=yrmu0)
#endif         
       ENDIF
...
!  swdown_vrai(1:knon) = swnet(1:knon)/(1. - albedo_keep(1:knon))
    ...
    IF (knon > 0) THEN
       ...
       CALL intersurf_main (itime+itau_phy, nbp_lon, nbp_lat, knon, ktindex, dtime,  &
            ...
            lon_scat, lat_scat, q2m, t2m, coszang=yrmu0(1:knon))
#endif       
    ENDIF
    z0h_new(:)=z0_new(:)
...
!
#endif
END MODULE surf_land_orchidee_noz0h_mod
